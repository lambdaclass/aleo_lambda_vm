...
mod tests {
    ...
    fn test_mul_with_u8_public_inputs() {
        let program_string = read_add_program("mul").unwrap();
        let (_, program) = Program::<Testnet3>::parse(&program_string).unwrap();
    ...
    fn test_is_eq_with_u8_public_inputs() {
        let program_string = read_add_program("is_eq").unwrap();
        let (_, program) = Program::<Testnet3>::parse(&program_string).unwrap();
        let function_name = "hello_10";
        let function = program
            .get_function(&Identifier::try_from(function_name).unwrap())
    ...
    fn test_ternary_with_u8_public_inputs_true_value() {
        let program_string = read_add_program("ternary").unwrap();
        let (_, program) = Program::<Testnet3>::parse(&program_string).unwrap();
        let function_name = "hello_10";
        let function = program
            .get_function(&Identifier::try_from(function_name).unwrap())
    ...
    fn test_ternary_with_u8_public_inputs_false_value() {
        ...
        */
        ...
        let (_program, program_build) = build_program(&program_string).unwrap();
        let function_identifier = Identifier::from_str(function_name).unwrap();
        let (_function_proving_ke1, function_verifying_key) =
            program_build.map.get(&function_identifier).unwrap();
        let public_inputs = user_inputs;
        assert!(verify_proof(function_verifying_key.clone(), &public_inputs, &proof).unwrap())
    }
