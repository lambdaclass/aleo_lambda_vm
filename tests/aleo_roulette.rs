...
mod aleo_roulette_functions_tests {
    ...
    fn assert_that_proof_for_function_execution_is_correct(
        ...
        let (_program, program_build) = lambdavm::build_program(&program.to_string()).unwrap();
    ...
    fn test_psd_hash() {
        ...
        let (function_variables, proof) =
            lambdavm::execute_function(&program, PSD_HASH, &user_inputs).unwrap();
    ...
    fn test_mint_casino_token_record() {
        ...
        let (function_variables, proof) =
            lambdavm::execute_function(&program, MINT_CASINO_TOKEN_RECORD, &user_inputs).unwrap();
    ...
    fn test_make_bet() {
        ...
        let (function_variables, proof) =
            lambdavm::execute_function(&program, MAKE_BET, &user_inputs).unwrap();
        ...
        assert!(matches!(r4, lambdavm::CircuitIOType::SimpleUInt64(_)));
        ...
        assert!(matches!(r9, lambdavm::CircuitIOType::SimpleUInt64(_)));
        assert_eq!(
            r9.value().unwrap(),
        ...
        assert!(matches!(r10, lambdavm::CircuitIOType::SimpleUInt64(_)));
        assert_eq!(
            r10.value().unwrap(),
        ...
        assert!(matches!(r11, lambdavm::CircuitIOType::SimpleUInt64(_)));
        assert_eq!(
            r11.value().unwrap(),
        ...
        assert!(matches!(r15, lambdavm::CircuitIOType::SimpleRecord(_)));
        if let lambdavm::CircuitIOType::SimpleRecord(record) = r15 {
    ...
    fn test_psd_bits_mod() {
        ...
        let (function_variables, proof) =
            lambdavm::execute_function(&program, PSD_BITS_MOD, &user_inputs).unwrap();
        ...
        // r3
        let r3 = function_variables["r3"].as_ref().unwrap();
        assert!(matches!(r3, lambdavm::CircuitIOType::SimpleBoolean(_)));
        assert_eq!(r3.value().unwrap(), "false");

        // r4
        let r4 = function_variables["r4"].as_ref().unwrap();
        assert!(matches!(r4, lambdavm::CircuitIOType::SimpleBoolean(_)));
        assert_eq!(r4.value().unwrap(), "false");

        // r5
        let r5 = function_variables["r5"].as_ref().unwrap();
        assert!(matches!(r5, lambdavm::CircuitIOType::SimpleBoolean(_)));
        assert_eq!(r5.value().unwrap(), "false");

        // r6
        let r6 = function_variables["r6"].as_ref().unwrap();
        assert!(matches!(r6, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r6.value().unwrap(), "0");

        // r7
        let r7 = function_variables["r7"].as_ref().unwrap();
        assert!(matches!(r7, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r7.value().unwrap(), "1");

        // r8
        let r8 = function_variables["r8"].as_ref().unwrap();
        assert!(matches!(r8, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r8.value().unwrap(), "2");

        // r9
        let r9 = function_variables["r9"].as_ref().unwrap();
        assert!(matches!(r9, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r9.value().unwrap(), "4");

        // r10
        let r10 = function_variables["r10"].as_ref().unwrap();
        assert!(matches!(r10, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r10.value().unwrap(), "8");

        // r11
        let r11 = function_variables["r11"].as_ref().unwrap();
        assert!(matches!(r11, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r11.value().unwrap(), "16");

        // r12
        let r12 = function_variables["r12"].as_ref().unwrap();
        assert!(matches!(r12, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r12.value().unwrap(), "32");

        // r13
        let r13 = function_variables["r13"].as_ref().unwrap();
        assert!(matches!(r13, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r13.value().unwrap(), "0");

        // r14
        let r14 = function_variables["r14"].as_ref().unwrap();
        assert!(matches!(r14, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r14.value().unwrap(), "0");

        // r15
        let r15 = function_variables["r15"].as_ref().unwrap();
        assert!(matches!(r15, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r15.value().unwrap(), "0");

        // r16
        let r16 = function_variables["r16"].as_ref().unwrap();
        assert!(matches!(r16, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r16.value().unwrap(), "0");

        // r17
        let r17 = function_variables["r17"].as_ref().unwrap();
        assert!(matches!(r17, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r17.value().unwrap(), "0");

        // r18
        let r18 = function_variables["r18"].as_ref().unwrap();
        assert!(matches!(r18, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r18.value().unwrap(), "0");

        // r19
        let r19 = function_variables["r19"].as_ref().unwrap();
        assert!(matches!(r19, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r19.value().unwrap(), "0");

        // r20
        let r20 = function_variables["r20"].as_ref().unwrap();
        assert!(matches!(r20, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r20.value().unwrap(), "0");

        // r21
        let r21 = function_variables["r21"].as_ref().unwrap();
        assert!(matches!(r21, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r21.value().unwrap(), "0");

        // r22
        let r22 = function_variables["r22"].as_ref().unwrap();
        assert!(matches!(r22, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r22.value().unwrap(), "0");

        // r23
        let r23 = function_variables["r23"].as_ref().unwrap();
        assert!(matches!(r23, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r23.value().unwrap(), "0");

        // r24
        let r24 = function_variables["r24"].as_ref().unwrap();
        assert!(matches!(r24, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r24.value().unwrap(), "0");

        // r25
        let r25 = function_variables["r25"].as_ref().unwrap();
        assert!(matches!(r25, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r25.value().unwrap(), "0");

        // r26
        let r26 = function_variables["r26"].as_ref().unwrap();
        assert!(matches!(r26, lambdavm::CircuitIOType::SimpleUInt16(_)));
        assert_eq!(r26.value().unwrap(), "0");

        // r27
        let r27 = function_variables["r27"].as_ref().unwrap();
        assert!(matches!(r27, lambdavm::CircuitIOType::SimpleBoolean(_)));
