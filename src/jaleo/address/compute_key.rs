...
impl TryFrom<&PrivateKey> for ComputeKey {
    ...
    fn try_from(private_key: &PrivateKey) -> Result<Self, Self::Error> {
        ...
        let pk_sig_x = pk_sig.x;
        let pr_sig_x = pr_sig.x;
        ...
        let mut sponge = PoseidonSponge::<Fp384<FqParameters>>::new(&sponge_param);
        sponge.absorb(&vec![pk_sig_x, pr_sig_x]);
        let sk_prf = *sponge.squeeze_field_elements(1).get(0).unwrap();
